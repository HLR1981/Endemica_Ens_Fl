...
if archivo:
    ...
    with st.spinner('Identificando...'):
        ...
        # EL ORDEN REAL SEGÚN TUS PRUEBAS DE CAMPO
        nombres_lista = [
            "encelia farinosa",              # 0
    ...
    # --- 6. RESULTADOS ---
    # Si la confianza es baja (menor al 48%), lo marcamos como desconocido
    if confianza < 48.0:
        st.error(f"### ⚠️ Imagen desconocida ({confianza:.2f}%)")
        st.info("La IA no está segura. Prueba acercando más la cámara a la planta.")
    else:
        # Buscamos la información en el diccionario usando el nombre de la lista
        clave_detectada = nombres_lista[idx]
        info = especies_info[clave_detectada]
